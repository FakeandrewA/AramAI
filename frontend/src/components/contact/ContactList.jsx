...
const ContactList = () => {
    const { getContacts, authUser, currentContact, setCurrentContact } = useAuthStore();
    ...
    useEffect(() => {
        const fetchContacts = async () => {
            const data = await getContacts(authUser._id);
            // console.log("Fetched raw contacts:", data);
            setContacts(data);
        };
        if (authUser?._id) fetchContacts();
    }, [authUser, getContacts]);

    // ✅ use actual property from your contact schema (username / fullName)
